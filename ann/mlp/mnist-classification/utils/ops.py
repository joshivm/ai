...
class ops:
    ...
    def dense(self,x,w_shape,b_shape,_x=False,_w=False,name="dense"):
        with tf.name_scope(name):
            with tf.variable_scope(name):
                ...
                return self.add(self.matmul(x,w,_w,_x),b)
    ...
    def optimizer(self,l,lr,ofn="SGD",var_list = None):
        ...
        if o is None:
            raise TypeError("Unknown value for the optimizer type, given "+ofn)

        return o

    def exponential_decay(self,start_lr,epochs,decay,staircase=True):
        global_step = tf.Variable(0, trainable=False)
        starter_learning_rate = start_lr
        learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step,
                                                   epochs, decay, staircase=staircase)
    ...
    def batch_eval(self,sess,opfn,pl_x,pl_y,x,y,b_len=5000,pl_train=False):
        ...
        for i in range(n_b):
            ...
            bn = {pl_x:x[start:last],pl_y:y[start:last]}
